...
public class BiggerThanExponent : MonoBehaviour {
	...
	void Start (){
		{
			...
			if (playerScript.summa < 100000) { // 10k-100k
				...
				summaBox = Mathf.Pow(summaX, summaY); // ger 256 - 823 543, funkar bra (t ex. 6^6=46 656) 
				SetBoxText ();
			}

			if (playerScript.summa > 100000 && playerScript.summa < 1000000 ) { // 100k - 1miljon

				summaX = Random.Range (5, 8);
				summaY = Random.Range (5, 8);
				summaBox = Mathf.Pow(summaX, summaY); // ger 3125 till 16 777 216
				SetBoxText ();
			}

			if (playerScript.summa > 1000000 && playerScript.summa < 800000000 ) { // 1miljon till 800milj
